<!doctype html>
<html lang="en">
	<head>
		<title>So Freo - Our products</title>
		<?php 
		$meta = 'Get some information about the veggie paté and more products soon';
		include 'includes/meta.php';
		?>
    </head>
    <body>
        <?php 
        include 'includes/nav.php';
        ?>
        <main>
            <div class="pic">
                <h1>Veggie Paté &amp; Co</h1>
                <div class="bold">
                <p>We are excited to share with you our new products : the Thai Veggie Paté and the Eggplant Jerky! </p>
                <p>New treats to add to your favorite vegan meal! <br>Stay connected!</p>
                </div>
            </div>
            <section class="row mx-md-3">
                <div class="col col-lg-one-fourth centertext"><h3>Veggie Paté</h3><a href="veggie-pate.php"><img src="img/veggie-pate-raw.jpg" alt="Veggie Paté"></a></div>
                <div class="col col-lg-one-fourth centertext"><h3>Thai Veggie Paté</h3><a href=""><img src="img/thai-veggie-pate.jpg" alt="Thai Veggie Paté"></a></div>
                <div class="col col-lg-one-fourth centertext"><h3>Eggplant Jerky</h3><a href=""><img src="img/eggplant-jerky.jpg" alt="Eggplant Jerky"></a></div>
                <div class="col col-lg-one-fourth centertext"><h3>Coming Soon</h3><a href=""><img src="img/coming-soon.svg" alt="Coming Soon"></a></div>
            </section>
        <?php include 'includes/footer.php'; ?>